# 
# Synthesis run script generated by Vivado
# 

set_msg_config -id {Common 17-41} -limit 10000000
set_msg_config -id {HDL 9-1061} -limit 100000
set_msg_config -id {HDL 9-1654} -limit 100000
create_project -in_memory -part xc7k70tfbv676-1

set_param project.singleFileAddWarning.threshold 0
set_param project.compositeFile.enableAutoGeneration 0
set_param synth.vivado.isSynthRun true
set_property webtalk.parent_dir {D:/MATERIALE AN 3 SEM 1/SSC/lab5/fifo/fifo.cache/wt} [current_project]
set_property parent.project_path {D:/MATERIALE AN 3 SEM 1/SSC/lab5/fifo/fifo.xpr} [current_project]
set_property default_lib xil_defaultlib [current_project]
set_property target_language Verilog [current_project]
set_property ip_output_repo {d:/MATERIALE AN 3 SEM 1/SSC/lab5/fifo/fifo.cache/ip} [current_project]
set_property ip_cache_permissions {read write} [current_project]
read_vhdl -library xil_defaultlib {{D:/MATERIALE AN 3 SEM 1/SSC/lab5/fifo/fifo.srcs/sources_1/new/fifo.vhd}}
foreach dcp [get_files -quiet -all *.dcp] {
  set_property used_in_implementation false $dcp
}
read_xdc {{D:/MATERIALE AN 3 SEM 1/SSC/const.xdc}}
set_property used_in_implementation false [get_files {{D:/MATERIALE AN 3 SEM 1/SSC/const.xdc}}]


synth_design -top fifo -part xc7k70tfbv676-1


write_checkpoint -force -noxdef fifo.dcp

catch { report_utilization -file fifo_utilization_synth.rpt -pb fifo_utilization_synth.pb }
